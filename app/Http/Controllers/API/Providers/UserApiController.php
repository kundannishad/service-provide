...
class UserApiController extends Controller
{
    ...
    */
    ...
    public function user_quote(Request $request)
    {
        $image='';
        $url = url("/");
        if ($request->file('image')) {
            ...
            foreach ($image as $file) {
                ...
                $filename = $i.'user' . time() . '.' . $file->getClientOriginalName();
                $location = 'public/uploads/user/';
            ...
            $booking->user_id = $request->user_id;
             ...
             $booking->user_requirement = $request->user_requirement;
             $booking->requirement_image = implode("|", $image);
             $booking->save();
             $response = array('error_code' =>0, 'message' => 'Booking Request insert successfully !','data'=>$booking);
...
$url = url("/");
        if ($user) {
            ...
            if($user->user_type == 1) {  //Service Provider
                ...
                foreach($bookings as $booking) {
                    ...
                    $image=explode('|',$booking->requirement_image);
                    $newimage=array();
            foreach ($image as $prow)
            {
                $newimage[]=$url.'/public/uploads/user/'.$prow;
            }
            $booking->requirement_image=$newimage;
    ...
    public function bookingDetailByProviderId(Request $request)
    {
        ...
        $image=explode('|',$bookingDetails->requirement_image);
                    $newimage=array();
            foreach ($image as $prow)
            {
                $newimage[]=$url.'/public/uploads/user/'.$prow;
            }
            $bookingDetails->requirement_image=$newimage;
        $singleUser = User::where('id',$bookingDetails->user_id)->with('userAddress')->first();
        $newimage=$url.'/public/uploads/user/'.$singleUser->image;
        $singleUser->image_url=$newimage;
        $bookingDetails->user_details=$singleUser;
    ...
    public function AddServiceProviderByCompanyId(Request $request) 
    { 
        $user = new User();
      ...
      //  $randomid = mt_rand(1000,9999); 
       ...
       // $this->sendSmsOnMobile($randomid,$request->phone_no);
        ...
        if($userData) {
            $location = 'public/uploads/user/';
            $image_path = $location.$userData->image;
            $userData->image_url= url($image_path);
            
            $serviceProviders =  $provider->getServiceProviderByUserId($userData->id);
            //$userData->service_detail = Service::where('id',$serviceProviders->service_id)->with('category')->first();
            $serviceData['service'] = Service::where('id',$serviceProviders->service_id)->first();
            $serviceData['category'] = Category::where('id',$serviceProviders->category_id)->first();
            $userData->service_detail = $serviceData;
    ...
    public function getCompanyServiceProviderById(Request $request)
    {
        ...
        $serviceData['service'] = Service::where('id',$serviceProviders->service_id)->first();
        $serviceData['category'] = Category::where('id',$serviceProviders->category_id)->first();
        $userDetails->service_detail = $serviceData;
    ...
    public function updateServiceProviderByCompanyID(Request $request)
    {
        $user = Auth::user();
        ...
        if($user->user_type == 2) {
            ...
            $validator = Validator::make($request->all(), [
                ...
                'image' => 'mimes:png,jpeg,jpg,gif',
                'phone_no' => 'numeric|min:10|unique:users,phone_no,'.$request->user_id,
                'email' =>    'min:10|email|unique:users,email,'.$request->user_id,
            ]);
            if ($validator->fails()) {
                $errorString = implode(",",$validator->messages()->all());
                return response()->json(['error_code' =>1,"message" => $errorString], $this->unauthorized);
            ...
            if($userDetails) {
                ...
                $update = $user->updateProfile($request->user_id,$data);
                ...
                $updateServiceByProviderId = $provider->updateServiceByProviderUserID($request->user_id,$serviData);

                if($updateServiceByProviderId) {
                
                    $serviceProviders =  $provider->getServiceProviderByUserId($request->user_id);
            ...
            return response()->json(['error_code' =>1,"message" => "Invalid company accesstoken"], $this->unauthorized);
        }
    }
   ...
   /**
    ...
    public function getServiceProvidersByCompanyId(Request $request)
    {
        $provider = new ServiceProvider();
        $companyId = Auth::user()->id;
        $userId = $request->user_id;
       ...
       foreach($serviceProviders as $key=>$serviceProvider) {
        ...
        $serviceProvider->rating_review = isset($rating_review)?$rating_review:"0";
            ...
            if(isset($serviceProvider->image)) {
                ...
                $serviceProvider->total_no_of_review= 0;
            } else {
                $serviceProvider->total_no_of_review= 0;
            }
